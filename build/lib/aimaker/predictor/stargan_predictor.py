...
class StarGANPredictor(BasePredictor):
    def __init__(self, config_path, source, transform, gpu_ids='', port=1234, divide_size=2):
        super(StarGANPredictor, self).__init__(source, transform, gpu_ids, port)
        config = configparser.ConfigParser()
        config.read(config_path)
        self.controller = util.ConfigHandler(config).getController()
    ...
    def predict(self, 
        ...
        for target in self.ds:
            ...
            for divided_target in self.divide_image(target):
                ...
                pred_lst  += [(self.controller.predict(divided_input, type_arr)[0] + 1) / 2]
