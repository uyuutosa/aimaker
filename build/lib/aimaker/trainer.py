...
class Trainer:
    def __init__(self, setting_path="settings"):
        ...
        self.n_update_graphs = self.sh.get_update_interval_of_graphs(self.dataset)
        self.n_update_images = self.sh.get_update_interval_of_images(self.dataset)
        if settings.data.base.isValid:
                ...
                if self.valid_dataset is not None:
                    if self.settings['data']['base']['isValid']:
                        ...
                        if current_epoch != 0 and not current_epoch % model_save_interval_valid:
                            self.controller._save_model(self.controller.get_model(), self.settings['validator']['base']['modelPath'], is_fcnt=False)
        ...
        for current_n_iter, data in enumerate(data_loader):
            ...
            try:
                ...
                if not current_n_iter % self.n_update_images:
                    if self.settings['base']['isView']:
                        self.viz.updateImages(self.controller.get_images(), current_n_iter)
