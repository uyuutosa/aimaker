...
class Pix2pixDividedGPUController:
    def __init__(self, setting):
        ...
        if setting['data']['base']['isTrain']:
            ...
            if len(self.gpu_ids):
                self.generator_criterion     = self.generator_criterion.to(self.gpu_ids[1])   
                self.discriminator_criterion = self.discriminator_criterion.to(self.gpu_ids[1])   
    ...
    def setInput(self, inputs, is_volatile=False):
        ...
        if len(self.gpu_ids):
            self.real                = self.real.to(self.gpu_ids[1])
            self.target              = self.target.to(self.gpu_ids[1])
            #if len(inputs) != 2:
            #    self.tf_map   = self.feature['tf_map'].to(self.gpu_ids[1])
    ...
    def forward(self):
        ...
        self.discriminator_loss = self._calcDiscriminatorLoss(real_AB, self.fake_AB, self.target)#.to(self.gpu_ids[0])
        
        if self.is_feature:
            input(self.is_feature)
    ...
    def _calcGeneratorLoss(self, fake, fake_AB, target):
        ...
        loss = self.discriminator_criterion(self._data_paralel_if_use(fake_AB.to(self.gpu_ids[0]), self.discriminator), True).to(self.gpu_ids[1])
    ...
    def _calcDiscriminatorLoss(self, real_AB, fake_AB, target):
        real_loss = self.discriminator_criterion(self._data_paralel_if_use(real_AB.to(self.gpu_ids[0]), self.discriminator), True)
        fake_loss = self.discriminator_criterion(self._data_paralel_if_use(fake_AB.detach().to(self.gpu_ids[0]), self.discriminator), False)
...
#                self.discriminator = torch.load(discriminator_name).to(self.gpu_ids[1])
                self.discriminator = torch.load(discriminator_name, map_location=lambda storage, loc: storage.cuda(convertDevice(self.gpu_ids[0])))
